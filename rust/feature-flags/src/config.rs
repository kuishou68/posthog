...
pub struct Config {
    ...
    #[envconfig(from = "BILLING_AGGREGATOR_FLUSH_INTERVAL_MS", default = "10000")]
    pub billing_aggregator_flush_interval_ms: u64,

    // Safety cap on pending entries — tripwire, not a working-set estimate.
    #[envconfig(from = "BILLING_AGGREGATOR_MAX_PENDING_ENTRIES", default = "500000")]
    pub billing_aggregator_max_pending_entries: usize,

    // Maximum `HIncrBy` commands per pipeline round-trip during a flush.
    #[envconfig(from = "BILLING_AGGREGATOR_PER_FLUSH_BATCH_SIZE", default = "200")]
    pub billing_aggregator_per_flush_batch_size: usize,
    ...
    #[envconfig(
        from = "BILLING_AGGREGATOR_SHUTDOWN_FLUSH_TIMEOUT_MS",
        default = "15000"
    )]
    pub billing_aggregator_shutdown_flush_timeout_ms: u64,
}
...
impl Config {
    ...
    pub fn default_test_config() -> Self {
        Self {
            ...
            billing_aggregator_flush_interval_ms: 10_000,
            billing_aggregator_max_pending_entries: 500_000,
            billing_aggregator_per_flush_batch_size: 200,
            billing_aggregator_shutdown_flush_timeout_ms: 15_000,
        }
    }
