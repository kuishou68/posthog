...
/// Verifies the billing aggregator writes (or, when `skip_writes=true`,
...
async fn test_flag_definitions_billing_counter(#[case] skip_writes: bool) {
    ...
    // recorded counter to Redis before the assertion.
    ...
    if skip_writes {
        // Wait long enough that a flush window has demonstrably elapsed
        // without a write — otherwise we'd be asserting on a race we
        // haven't run yet.
