...
describe('Tasks', { concurrent: false }, () => {
    ...
    describe('tasks-runs-retrieve tool', () => {
        // Cannot reliably create a TaskRun via MCP (tasks-run-create is intentionally disabled,
        // and runs are produced by a Temporal workflow). This test discovers any existing run
        // in the test project and asserts the tool path executes cleanly. If no runs exist,
        // the test still passes — verifying at minimum that the discovery call did not error.
