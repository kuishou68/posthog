...
class LogsAlertViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def simulate(self, request: Request, *args: object, **kwargs: object) -> Response:
        ...
        # Simulate samples at the alert's actual check cadence so the preview matches
        # what production would do. Each bucket is one cadence period of data.
        ...
        # Fill gaps so every cadence tick has a count (0 if no logs)
        cadence_buckets = _fill_empty_buckets(sparse_buckets, date_from_dt, date_to_dt, DEFAULT_CHECK_INTERVAL_MINUTES)

        # Rolling window: each bucket holds DEFAULT_CHECK_INTERVAL_MINUTES of data, so the alert's
        # window_minutes aggregates the trailing window_minutes/cadence buckets. For default
        # window=cadence (both 5), each bucket IS the count and the rolling sum is a no-op.
        buckets_per_window = max(1, window_minutes // DEFAULT_CHECK_INTERVAL_MINUTES)
